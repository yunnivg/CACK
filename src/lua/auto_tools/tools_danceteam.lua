            ...
            if not nx_is_valid(view) then
                ...
                if cur_state == 1 then
                    ...
                    if nx_int(row_count) < cfg_start_condition then
                        ...
                        -- Rao luyện nhóm
                        if tools_difftime(last_chat) > 19 then
...
function on_main_form_open(form)
    ...
    local html = nx_function("ext_utf8_to_widestr", "LC 25 TGT còn xxx slot!")
...
function on_btn_jonner_click(btn)
    ...
    if auto_is_running then
        auto_is_running = false
        btn.Text = nx_function("ext_utf8_to_widestr", "Ké luyện nhóm")
        form.btn_leader.Visible = true
    else
        auto_is_running = true
        btn.Text = nx_function("ext_utf8_to_widestr", "Dừng")
