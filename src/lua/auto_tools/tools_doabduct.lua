            ...
            if logicstate == 1 and isStopIfBeattacked then
                ...
                if not isLoggedAttacker then
                    ...
                    for i = 1, totalAttacker do
                        ...
                        consoleAttacker(tostring(os.date("%c")) .. ": " .. listAttacker[i])
                        textChatLog = textChatLog .. nx_function("ext_utf8_to_widestr", listAttacker[i])
...
function get_abduct_data(city, pos)
	-- Thiên đăng trấn
	if city == "born04" then
		-- Cổng Diệp Gia
...
function on_typestatic_click(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
function on_combobox_time_selected(combobox)
    stop_type_static()
end
