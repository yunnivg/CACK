...
function main_form_open(self)
  ...
  local l, t = load_update_location()
  form.groupbox_update.Top = t
  form.groupbox_update.Left = l
  local l, t = load_dbomall_location()
  form.groupbox_dbomall.Top = t
  form.groupbox_dbomall.Left = l
  form:ToFront(form.groupbox_update)
...
function change_size()
  ...
  if nx_find_custom(form, "groupbox_update") and nx_is_valid(form.groupbox_update) and (form.groupbox_update.Top + form.groupbox_update.Height > gui.Height or form.groupbox_update.Left + form.groupbox_update.Width > gui.Width) then
    form.groupbox_update.Top = 0
    form.groupbox_update.Left = 359
  end
...
function on_groupbox_update_get_capture(groupbox)
  groupbox.BackImage = "gui\\common\\form_line\\mibox_1.png"
end
function on_groupbox_update_lost_capture(groupbox)
  groupbox.BackImage = ""
end
function save_active_location()
  ...
  ini:LoadFromFile()
  local groupbox_update = form.groupbox_update
  ini:WriteInteger("update", "Left", nx_int(groupbox_update.Left))
  ini:WriteInteger("update", "Top", nx_int(groupbox_update.Top))
...
function update_newjh_groupbox(form)
  ...
  if bIsNewJHModule then
    ...
    form.groupbox_update.Visible = true
...
function is_show_donghai_activity_btn()
  local scene_xaikedao = 603
  local scene_penglaidao = 604
  local scene_wujidoa = 605
  local game_client = nx_value("game_client")
  if not nx_is_valid(game_client) then
    return
  end
  ...
  local cur_scene_id = nx_number(get_scene_id())
  if cur_scene_id < 0 then
    return
  end
  ...
  if cur_scene_id == scene_xaikedao or cur_scene_id == scene_penglaidao or cur_scene_id == scene_wujidoa then
    grp.Visible = true
    grp.Fixed = false
  else
    grp.Visible = false
