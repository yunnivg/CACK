...
local tools_btns_data = {
 {
	...
	{
		"24", 
		"gui\\special\\btn_main40rh\\btn_sysytem_on.png", 
		"Thiết lập theo acc", 
		"auto_tools\\tools_config", "", 
		1, 
		false
	}, 
	{
		"25",
		"gui\\special\\zhenqi\\jingmai_on.png", "Đổi kinh mạch nhanh", 
		"auto_tools\\tools_jingmai", 
		"", 
		1, 
		false
	}, 
    {
        "26", -- Auto mua đồ ở chợ
    ...
    {
        "28", -- Auto hào kiệt
        "gui\\special\\btn_main\\smdh_on.png",
        "Auto hào kiệt",
        "auto_tools\\tools_jhboss",
        "",
        1,
        false
    },
	    {
        "30", -- Hồi Sinh
        "gui\\special\\btn_main40rh\\btn_trade_down.png",
        "Hồi Sinh-",
        "auto_tools\\tools_reborn.lua",
        "auto_init",
        0,
        false
    },
...
on_form_main_init = function(self)
  self.start_timer = false
end
on_main_form_open = function(self)
  ...
  local btn_count_cal = 0
  local offsetJ = 0
  for i = 1, btn_count do
    -- Quyền Admin
    if not tools_btns_data[i][7] or isAdmRights() then
        btn_count_cal = btn_count_cal + 1
        offsetJ = offsetJ + 1
  ...
  local groupscrollbox_height = getHeightContainer(btn_count_cal, 3)
...
on_groupscrollbox_1_get_capture = function(self)
  local form_main = nx_value("form_stage_main\\form_main\\form_main")
  if not nx_is_valid(form_main) then
    return
  end
...
on_groupscrollbox_2_get_capture = function(self)
  local form_main = nx_value("form_stage_main\\form_main\\form_main")
  if not nx_is_valid(form_main) then
    return
  end
...
on_groupscrollbox_3_get_capture = function(self)
  local form_main = nx_value("form_stage_main\\form_main\\form_main")
  if not nx_is_valid(form_main) then
    return
  end
...
on_main_form_close = function(self)
...
on_imagegrid_sel_lost_capture = function(grid)
...
on_imagegrid_sel2_lost_capture = function(grid)
  local form = grid.ParentForm
  local left = form.AbsLeft
  local top = form.AbsTop
  local right = left + form.Width
...
on_imagegrid_sel3_lost_capture = function(grid)
  local form = grid.ParentForm
  local left = form.AbsLeft
  local top = form.AbsTop
  local right = left + form.Width
...
on_imagegrid_sel_get_capture = function(grid)
  local form_main = nx_value("form_stage_main\\form_main\\form_main")
  nx_execute("form_stage_main\\form_main\\form_main", "on_del_hide_func_timer", form_main)
  local timer = nx_value(GAME_TIMER)
  if nx_is_valid(timer) then
...
on_imagegrid_sel2_get_capture = function(grid)
  local form_main = nx_value("form_stage_main\\form_main\\form_main")
  nx_execute("form_stage_main\\form_main\\form_main", "on_del_hide_func_timer", form_main)
  local timer = nx_value(GAME_TIMER)
  if nx_is_valid(timer) then
...
on_imagegrid_sel3_get_capture = function(grid)
  local form_main = nx_value("form_stage_main\\form_main\\form_main")
  nx_execute("form_stage_main\\form_main\\form_main", "on_del_hide_func_timer", form_main)
  local timer = nx_value(GAME_TIMER)
  if nx_is_valid(timer) then
...
on_imagegrid_sel_select_changed = function(grid, index)
...
on_imagegrid_sel2_select_changed = function(grid, index)
  local selnumber = tonumber(grid.sel_name)
  local btnsnum = table.getn(tools_btns_data)
  for i = 1, btnsnum do
    local data = tools_btns_data[i]
...
on_imagegrid_sel3_select_changed = function(grid, index)
  local selnumber = tonumber(grid.sel_name)
  local btnsnum = table.getn(tools_btns_data)
  for i = 1, btnsnum do
    local data = tools_btns_data[i]
...
on_create_groupbox = function(form, x, y, image, text, name, sel_index)
  if not nx_is_valid(form) then
    return
  end
  local gui = nx_value("gui")
...
on_groupbox_get_capture = function(groupbox)
  if not nx_is_valid(groupbox) then
    return
  end
...
on_groupbox2_get_capture = function(groupbox)
  if not nx_is_valid(groupbox) then
    return
  end
  nx_execute("tips_game", "hide_tip")
...
on_groupbox3_get_capture = function(groupbox)
  if not nx_is_valid(groupbox) then
    return
  end
  nx_execute("tips_game", "hide_tip")
...
getHeightContainer = function(num, perrow)
  local surplus = num % perrow
  local he = math.floor(num / perrow)
  if surplus == 0 then
    return he
  end
  return he + 1
