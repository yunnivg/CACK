...
-- 
getDistanceWithObj = function(pos, vobj)
...
auto_run_acceptrequest = function()
    require("define\\request_type")
    require("share\\client_custom_define")
    local FORM_MAIN_REQUEST = "form_stage_main\\form_main\\form_main_request"
			...
			if num_request > 0 then
				for i = 1, num_request do
					...
					if request_type == REQUESTTYPE_INVITETEAM then
                        ...
                        break
					elseif request_type == REQUESTTYPE_BATTLEFIELD_SWITCH or request_type == REQUESTTYPE_BATTLEFIELD_WRESTLE_HELP then
						-- Chấp nhận lời mời vào ct
                            isAccepttedRequest = true
...
get_player_realschool = function()
    local game_client = nx_value("game_client")
    if not nx_is_valid(game_client) then
        return false
    end
...
show_spy_task_detail = function(task_data)
	local form = nx_value(THIS_FORM)
	if not nx_is_valid(form) then
		return false
	end
...
get_map_spy = function(task_data)
    local game_client = nx_value("game_client")
    if not nx_is_valid(game_client) then
        return false
    end
...
check_valid_skill_mkth = function()
...
get_max_JiangHuHomePoint = function()
    local game_client = nx_value("game_client")
    if not nx_is_valid(game_client) then
        return 0
    end
...
MoveToMapByHomePoint = function(hp)
    local game_client = nx_value("game_client")
    if not nx_is_valid(game_client) then
        return 0
    end
...
get_player_home_uniquemap = function()
    return array_home_unique_map[get_player_realschool()]
end

-- Tự tử
suicidePlayer = function()
    if get_buff_info("buf_riding_01") ~= nil then
        nx_execute("custom_sender", "custom_remove_buffer", "buf_riding_01")
        nx_pause(0.2)
    end
...
detect_intelligence_npc = function()
    local game_client = nx_value("game_client")
    if not nx_is_valid(game_client) then
        return false, false
    end
...
control_select_npc = function(npc)
    local game_client = nx_value("game_client")
    if not nx_is_valid(game_client) then
        return false
    end
...
on_btn_control_rsabduct_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_khd_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_buff_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_player_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_findpath_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_offfree_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_trace_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_attack_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_hgbc_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_dddh_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_ntlm_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_delmail_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_catchabduct_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_noterequest_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_acceptrequest_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_spy_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_vptd_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
on_btn_control_capbird_click = function(btn)
	local form = btn.ParentForm
	if not nx_is_valid(form) then
		return
	end
...
tools_resume_scene_music = function(cfg)
	local role = nx_value("role")
	if not nx_is_valid(role) then
		return
	end
...
on_buff_imagegrid_mousein_grid = function(grid, index)
...
on_buff_imagegrid_mouseout_grid = function(grid, index)
